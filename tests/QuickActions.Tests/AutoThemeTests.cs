...
public class AutoThemeSettingsTests
{
    ...
    [Fact]
    public void FromArgs_MissingConfig_FallsBackToJinan()
    {
    ...
    {
        // 参考值由 NOAA 算法独立实现（Python）计算，±5 分钟容差
        var settings = AutoThemeSettings.FromArgs(null);
    ...
    [Fact]
    public void FromArgs_OnlyOneCoordinate_FallsBackToJinan()
    {
        // 只给 latitude：不构成有效坐标对，回退内置济南坐标（不报错，开箱即用）
