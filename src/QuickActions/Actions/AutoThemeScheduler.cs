...
/// <summary>
/// 自动亮暗切换的设置：默认内置北京坐标（未配置即用），需要覆盖时在 config.json 加
...
{
    /// <summary>内置默认坐标：北京（未配置时使用；时区固定北京时间，不随坐标/机器变化）。</summary>
...
/// <summary>
/// 按日出日落自动切换亮/暗的后台调度器。启用状态持久化到 HKCU\Software\QuickActions\AutoTheme 的
...
{
    private const string FlagKey = @"HKEY_CURRENT_USER\Software\QuickActions\AutoTheme";
    ...
    public AutoThemeScheduler(IAction themeAction, AutoThemeSettings settings, Logger log)
    {
    ...
    private void OnTick()
    {
        ...
        catch (Exception ex)
        {
            ...
            _timer.Interval = (int)TimeSpan.FromHours(1).TotalMilliseconds;
        }
    }
